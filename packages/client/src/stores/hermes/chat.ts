...
/** Normalize message content to a string. Handles Anthropic-style array content blocks. */
...
function mapHermesMessages(msgs: HermesMessage[]): Message[] {
  ...
  const filteredMsgs = msgs.filter(m => {
    if (m.role === 'assistant') {
      ...
      return (m.tool_calls?.length || 0) > 0 || text.trim() !== ''
    }
    return true
  })
  ...
  for (const msg of filteredMsgs) {
    ...
    if (msg.role === 'assistant' && msg.tool_calls?.length && !normalizeContent(msg.content).trim()) {
    ...
    if (msg.role === 'tool') {
      ...
      result.push({
        ...
        toolResult: contentStr || undefined,
        toolStatus: 'done',
      })
      continue
    }
    ...
    document.addEventListener('visibilitychange', () => {
      if (document.visibilityState === 'visible' && activeSessionId.value && !isStreaming.value) {
        ...
        if (sid && !streamStates.value.has(sid)) {
          ...
          }, undefined, activeSession.value?.profile)
        }
      }
    })
  }
