...
export const useGroupChatStore = defineStore('groupChat', () => {
    ...
    function connect() {
        ...
        socket.on('agent_mention_toggled', (data: { roomId: string; enabled: boolean }) => {
            const room = rooms.value.find(r => r.id === data.roomId)
            if (room) room.agentMentionEnabled = data.enabled ? 1 : 0
        })

        socket.on('custom_rules_updated', (data: { roomId: string; customRules: string }) => {
            const room = rooms.value.find(r => r.id === data.roomId)
            if (room) room.customRules = data.customRules
        })
    }
    ...
    async function createNewRoom(name: string, inviteCode: string, agentList?: { profile: string; name?: string; description?: string; invited?: boolean }[], compression?: { triggerTokens: number; maxHistoryTokens: number; tailMessageCount: number }, customRules?: string) {
        try {
    ...
    async function toggleAgentMention(enabled: boolean) {
        const socket = getSocket()
        if (!socket || !currentRoomId.value) return
        await new Promise<void>((resolve, reject) => {
            socket.emit('toggle_agent_mention', { roomId: currentRoomId.value, enabled }, (res: any) => {
                if (res?.error) reject(new Error(res.error))
                else resolve()
            })
        })
    ...
    async function updateCustomRules(customRules: string) {
        const socket = getSocket()
        if (!socket || !currentRoomId.value) return
        await new Promise<void>((resolve, reject) => {
            socket.emit('update_custom_rules', { roomId: currentRoomId.value, customRules }, (res: any) => {
                if (res?.error) reject(new Error(res.error))
                else resolve()
            })
        })
