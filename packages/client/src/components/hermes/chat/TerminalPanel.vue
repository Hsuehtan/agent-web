...
function buildSocketIOPath(): string {
  const base = getBaseUrlValue();
  if (base) {
    const url = new URL(base);
    return url.pathname.replace(/\/$/, "") + "/socket.io";
  }
  return "/socket.io";
}
...
function connect() {
  ...
  try {
    socketIOClient = io(`${socketUrl}/terminal`, {
      path: buildSocketIOPath(),
      auth: { token },
    ...
    console.error("[Terminal] Socket.IO creation failed:", err);
  ...
  socketIOClient.on("terminal:data", (data: string) => {
  ...
  socketIOClient.on("connect_error", (err) => {
    console.error("[Terminal] Socket.IO connect error:", err.message);
    isConnecting.value = false;
    scheduleReconnect();
  });
}
...
function send(data: object | string) {
  if (!socketIOClient?.connected) return;
  if (typeof data === "string") {
    socketIOClient.emit("terminal:input", data);
  } else {
    socketIOClient.emit("terminal:control", JSON.stringify(data));
  }
}
...
function sendResize() {
  if (!activeTerm || !socketIOClient?.connected) return;
  try {
    send({
      type: "resize",
...
watch(() => props.visible, (visible) => {
  if (visible && !hasConnected && !socketIOClient) {
    hasConnected = true;
    connect();
  }
}, { immediate: true });
