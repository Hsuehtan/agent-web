...
// --- System bridge commands (static) ---
const systemCommands = computed(() => [
  { name: 'usage', args: '', description: t('chat.slashCommands.usage'), isSkill: false },
  { name: 'status', args: '', description: t('chat.slashCommands.status'), isSkill: false },
  { name: 'abort', args: '', description: t('chat.slashCommands.abort'), isSkill: false },
  { name: 'queue', args: t('chat.slashCommandArgs.message'), description: t('chat.slashCommands.queue'), isSkill: false },
  { name: 'clear', args: '', description: t('chat.slashCommands.clear'), isSkill: false },
  { name: 'clear', args: '--history', insertText: 'clear --history', description: t('chat.slashCommands.clearHistory'), isSkill: false },
  { name: 'title', args: t('chat.slashCommandArgs.title'), description: t('chat.slashCommands.title'), isSkill: false },
  { name: 'compress', args: '', description: t('chat.slashCommands.compress'), isSkill: false },
  { name: 'steer', args: t('chat.slashCommandArgs.text'), description: t('chat.slashCommands.steer'), isSkill: false },
  { name: 'destroy', args: '', description: t('chat.slashCommands.destroy'), isSkill: false },
])

// --- Skill commands (fetched from API) ---
...
async function loadSkillCommands() {
  try {
    ...
    // Silently ignore — skills are optional in the slash menu
  }
}

// Merged: system commands first, then skill commands
const bridgeCommands = computed<SlashCommand[]>(() => [
...
const filteredBridgeCommands = computed(() => {
  ...
  return bridgeCommands.value.filter(command =>
    ...
    || command.insertText?.toLowerCase().includes(query)
...
onMounted(() => {
  ...
  // Load skill commands for slash menu
  loadSkillCommands()
})
  ...
  <div class="chat-input-area">
    ...
    >
      ...
      <Transition name="dropdown-fade">
        ...
        >
          <template v-for="(command, i) in filteredBridgeCommands" :key="`${command.name}-${command.args}-${i}`">
            <div
              v-if="command.isSkill && i > 0 && !filteredBridgeCommands[i - 1].isSkill"
              class="slash-command-divider"
            >
              <span class="slash-command-divider-label">{{ t('chat.slashCommands.skillSection') }}</span>
            </div>
            <div
              class="slash-command-item"
              :class="{ active: i === slashActiveIndex, 'is-skill': command.isSkill }"
              @mousedown.prevent="selectBridgeCommand(command)"
              @mouseenter="handleCommandHover(i)"
            >
              <span class="slash-command-name">/{{ command.insertText || command.name }}</span>
              <span v-if="command.args" class="slash-command-args">{{ command.args }}</span>
              <span v-if="command.category && command.isSkill" class="slash-command-category">{{ command.category }}</span>
