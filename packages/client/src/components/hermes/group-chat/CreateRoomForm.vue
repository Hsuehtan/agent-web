...
const emit = defineEmits<{
    submit: [name: string, inviteCode: string, userName: string, description: string, compression: { triggerTokens: number; maxHistoryTokens: number; tailMessageCount: number }, customRules: string]
    cancel: []
}>()
...
function handleCreate() {
    ...
    if (!name || !user) return
    emit('submit', name, code, user, description.value.trim(), { ...compression.value }, customRules.value.trim())
}
