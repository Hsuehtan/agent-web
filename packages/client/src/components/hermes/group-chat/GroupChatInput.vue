...
onMounted(() => {
    ...
    if (room) {
        agentMentionEnabled.value = !!room.agentMentionEnabled
    }
})
...
watch(agentMentionEnabled, async (value) => {
    try {
        await store.toggleAgentMention(value)
    } catch {
        // Revert on error
        agentMentionEnabled.value = !value
    }
})
        ...
        <div class="input-top-bar">
            ...
            <div class="auto-play-speech-switch">
                <NTooltip trigger="hover">
                    ...
                    {{ t('groupChat.agentCollaboration') }}
                </NTooltip>
                <NSwitch v-model:value="agentMentionEnabled" size="small" :round="false" />
            </div>
            <NTooltip trigger="hover">
                <template #trigger>
