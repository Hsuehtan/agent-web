...
const showAgentDescModal = ref(false)
...
<template>
    <div v-if="isToolMessage" class="group-message tool-message">
        ...
        <div class="msg-body">
            <div class="msg-header">
                ...
                <span v-if="isAgent && agentInfo?.description" class="agent-desc-toggle" @click.stop="showAgentDescModal = true" :title="t('chat.viewAgentDescription')">
                    <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
                        <rect x="3" y="4" width="18" height="16" rx="2"/>
                        <circle cx="9" cy="10" r="2"/>
                        <path d="M15 8h2M13 12h4"/>
                        <path d="M6 17c0-1.5 1.35-3 3-3s3 1.5 3 3"/>
                    </svg>
                </span>
            </div>
    ...
    <NModal
        v-model:show="showAgentDescModal"
        preset="card"
        :title="agentInfo?.name || message.senderName"
        :style="{ maxWidth: '520px', width: '90vw' }"
        :bordered="false"
        :segmented="{ content: true }"
        :closable="true"
    >
        <div class="agent-desc-modal-content">
            <div class="agent-desc-modal-profile">
                <ProfileAvatar :name="avatarProfileName" :avatar="avatarProfile?.avatar" :size="48" />
                <div class="agent-desc-modal-meta">
                    <span class="agent-desc-modal-name">{{ message.senderName }}</span>
                    <span v-if="agentInfo?.profile" class="agent-desc-modal-profile-label">{{ agentInfo.profile }}</span>
                </div>
            </div>
            <div class="agent-desc-modal-text">{{ agentInfo?.description }}</div>
        </div>
    </NModal>
</template>
...
.agent-desc-modal-content {
    .agent-desc-modal-profile {
        display: flex;
        align-items: center;
        gap: 12px;
        margin-bottom: 16px;
    ...
    .agent-desc-modal-meta {
        display: flex;
        flex-direction: column;
        gap: 2px;
    }

    .agent-desc-modal-name {
        font-size: 15px;
        font-weight: 600;
        color: var(--text-primary);
    }

    .agent-desc-modal-profile-label {
        font-size: 12px;
        color: $text-muted;
