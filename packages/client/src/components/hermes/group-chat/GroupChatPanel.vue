...
async function handleCreateRoom(name: string, inviteCode: string, userName: string, description: string, compression: { triggerTokens: number; maxHistoryTokens: number; tailMessageCount: number }, customRules: string) {
    try {
        store.setUserInfo(userName, description)
        const res = await store.createNewRoom(name, inviteCode, undefined, compression, customRules)
...
function handleOpenCompressionConfig() {
    ...
    if (room) {
        ...
        customRulesText.value = room.customRules ?? ''
    }
...
async function handleSaveCompressionConfig() {
    if (!store.currentRoomId) return
    try {
        const res = await updateRoomConfig(store.currentRoomId, { ...compressionConfig.value, customRules: customRulesText.value })
                ...
                <div class="modal">
                    ...
                    <div class="form-group">
                        ...
                        <NInput
                            ...
                            show-count
                        />
                        <p class="form-hint">{{ t('groupChat.customRulesDesc') }}</p>
                    </div>
