...
import { readConfigYaml, readConfigYamlForProfile, updateConfigYaml, updateConfigYamlForProfile, fetchProviderModels, buildModelGroups, PROVIDER_ENV_MAP, ensureConfigYamlExists } from '../../services/config-helpers'
...
async function buildAvailableForProfile(
  ...
  // Ensure config.yaml exists so the UI can read/write model configuration
  await ensureConfigYamlExists(profile)
