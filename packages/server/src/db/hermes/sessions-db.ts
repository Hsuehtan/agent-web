...
/** Check if the Hermes Agent state.db exists for the given (or active) profile. */
function stateDbExists(profile?: string): boolean {
...
async function openSessionDb(profile?: string): Promise<InstanceType<typeof import('node:sqlite').DatabaseSync> | null> {
  if (!SQLITE_AVAILABLE) {
    throw new Error(`node:sqlite requires Node >= 22.5, current: ${process.versions.node}`)
  }
  if (!stateDbExists(profile)) {
    return null
  }
