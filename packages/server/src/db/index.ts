...
// Use config.appHome (writable user data dir) for all non-test environments.
// Previously, NODE_ENV !== 'production' was used to pick a cwd-relative path,
// but deployment environments may not set NODE_ENV, causing mkdir to fail in
// read-only deployment directories (e.g. /opt/bytefaas/packages/server/data).
const DB_DIR = isTest
