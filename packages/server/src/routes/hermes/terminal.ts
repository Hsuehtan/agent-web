...
// ─── Transport-agnostic send interface ─────────────────────────
...
// ─── Shared terminal logic (transport-agnostic) ────────────────

function handleTerminalConnection(transport: Transport, defaultShell: string) {
  ...
  function attachPtyOutput(session: PtySession) {
    ...
    session.pty.onExit(({ exitCode }: { exitCode: number }) => {
      ...
      if (transport.isOpen()) {
        transport.send(JSON.stringify({ type: 'exited', id: session.id, exitCode }))
      }
  ...
  function handleControl(parsed: any) {
    switch (parsed.type) {
      ...
      case 'switch': {
        ...
        if (!session) {
          transport.send(JSON.stringify({ type: 'error', message: 'Session not found' }))
          return
        }
        conn.activeSessionId = sessionId

        // Send switched first so frontend mounts the correct terminal
        transport.send(JSON.stringify({ type: 'switched', id: sessionId }))
...
export function setupTerminalWebSocket(httpServers: HttpServer | HttpServer[]) {
  ...
  wss.on('connection', (ws) => {
    const transport: Transport = {
      ...
      onMessage: (cb) => ws.on('message', (raw) => { cb(Buffer.isBuffer(raw) ? raw.toString('utf8') : String(raw)) }),
    }
    handleTerminalConnection(transport, defaultShell)
  })

  logger.info('WebSocket ready at /terminal (shell: %s, transport: node-pty)', defaultShell)
}

// ─── Socket.IO terminal namespace setup ─────────────────────────

export function setupTerminalSocketIO(io: SocketIOServer) {
  if (!pty) {
    logger.warn('node-pty not available, skipping terminal Socket.IO setup')
    return
  }
  ...
  // Auth middleware — same pattern as chat-run
  nsp.use(async (socket: SocketIOSocket, next: (err?: Error) => void) => {
  ...
  nsp.on('connection', (socket: SocketIOSocket) => {
    logger.info('Terminal Socket.IO client connected: %s', socket.id)
    const transport: Transport = {
      send: (data: string) => { socket.emit('terminal:data', data) },
      isOpen: () => socket.connected,
      onClose: (cb) => socket.on('disconnect', cb),
      onMessage: (cb) => {
        socket.on('terminal:input', (data: string) => { cb(data) })
        socket.on('terminal:control', (data: string) => { cb(data) })
      },
    }
    handleTerminalConnection(transport, defaultShell)
  })

  logger.info('Socket.IO terminal ready at /terminal (shell: %s, transport: node-pty)', defaultShell)
}
