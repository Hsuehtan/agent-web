...
downloadRoutes.get('/api/hermes/download', async (ctx) => {
  ...
  try {
    ...
    // Use 'inline' for viewable types (images, videos, audio, PDFs) so <img>/<video> tags can render them.
    // Use 'attachment' for all other types to trigger a download.
    const viewablePrefixes = ['image/', 'video/', 'audio/', 'text/', 'application/pdf']
    const disposition = viewablePrefixes.some(p => mime.startsWith(p)) ? 'inline' : 'attachment'
