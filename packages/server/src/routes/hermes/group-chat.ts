...
groupChatRoutes.post('/api/hermes/group-chat/rooms', async (ctx) => {
    ...
    const { name, inviteCode, agents, compression, customRules } = ctx.request.body as {
        name?: string
        inviteCode?: string
    ...
    storage.saveRoom(roomId, name, inviteCode, compression, customRules)
...
groupChatRoutes.put('/api/hermes/group-chat/rooms/:roomId/config', async (ctx) => {
    ...
    const roomId = ctx.params.roomId
    const { triggerTokens, maxHistoryTokens, tailMessageCount, agentMentionEnabled, mentionDepthLimit, customRules } = ctx.request.body as {
    ...
    chatServer.getStorage().updateRoomConfig(roomId, { triggerTokens, maxHistoryTokens, tailMessageCount, agentMentionEnabled, mentionDepthLimit, customRules })
