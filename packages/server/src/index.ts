// Force enable authentication: clear AUTH_DISABLED injected by the platform
...
import { setupTerminalWebSocket, setupTerminalSocketIO } from './routes/hermes/terminal'
...
export async function bootstrap() {
  ...
  // Terminal Socket.IO — uses the same Server instance for CDN/proxy compatibility
  setupTerminalSocketIO(groupChatServer.getIO())
  ...
  // GatewayManager — required for proxy routes and model config persistence
  const { GatewayManager } = await import('./services/hermes/gateway-manager')
  const gatewayManager = new GatewayManager(activeProfile)
  const { setGatewayManagerForTest } = await import('./routes/hermes/proxy-handler')
  setGatewayManagerForTest(gatewayManager)
  console.log('[bootstrap] GatewayManager initialized, profile=%s', activeProfile)
