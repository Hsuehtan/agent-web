...
/**
 * Ensure that config.yaml exists for the given profile (or active profile).
 * If missing, create a minimal scaffold so the UI can read/write model config.
 */
export async function ensureConfigYamlExists(profile?: string): Promise<void> {
  const configPath = profile ? configPathForProfile(profile) : getActiveConfigPath()
  ...
  await safeFileStore.writeYaml(configPath, minimalConfig)
  logger.info(`[config] Created minimal config.yaml at ${configPath}`)
}
