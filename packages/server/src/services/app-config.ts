...
export async function readAppConfig(): Promise<AppConfig> {
  ...
  try {
    ...
    // Do NOT cache the empty fallback — leave cache as null so that
    // subsequent calls will re-read the file if it gets created later
    // (e.g. after a config write from the UI).
    return {}
  }
}
