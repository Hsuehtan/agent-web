...
function hermesBinPython(): string | undefined {
  ...
  try {
    ...
    // Handle shebangs like #!/usr/bin/env python3 — the actual interpreter
    // is the argument after `env`, not `env` itself.  `env -S` also accepted.
    if (base === 'env') {
      // Skip env flags (e.g. -S) to find the real executable name
      let i = 1
      while (i < parts.length && parts[i].startsWith('-')) i++
      const realExe = parts[i]
      if (realExe) {
        const resolved = resolveExecutable(realExe)
        if (resolved) return resolved
      }
      // env wrapper without a resolvable target — let fallback handle it
      return undefined
    }
    return parts[0] && existsSync(parts[0]) ? parts[0] : undefined
  } catch {
    return undefined
  }
}
...
export class AgentBridgeManager {
  ...
  private async startProcess(): Promise<void> {
    ...
    const pylibsDir = agentRoot ? resolve(agentRoot, '..', 'pylibs') : ''
    const env = {
      ...process.env,
      HERMES_AGENT_BRIDGE_ENDPOINT: this.endpoint,
      HERMES_HOME: hermesHome,
      ...(agentRoot ? { HERMES_AGENT_ROOT: agentRoot } : {}),
      ...(pylibsDir && existsSync(pylibsDir)
        ? { PYTHONPATH: [pylibsDir, process.env.PYTHONPATH].filter(Boolean).join(':') }
        : {}),
    }
