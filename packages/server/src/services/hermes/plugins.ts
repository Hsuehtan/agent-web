...
export async function listHermesPlugins(profile?: string): Promise<HermesPluginsResponse> {
  ...
  // When agentRoot is not resolved, hermes-agent Python modules (hermes_cli,
  // hermes_constants, etc.) are unavailable. Running the PYTHON_BRIDGE would
  // fail with an ImportError (exit code 2) and produce a 500 response.
  // Gracefully degrade instead: return an empty plugin list with a warning.
  if (!agentRoot) {
    return {
      plugins: [],
      warnings: ['Hermes Agent not found — plugin discovery is unavailable. Install or configure HERMES_AGENT_ROOT to enable plugins.'],
      metadata: {
        hermesAgentRoot: '',
  ...
  // In production FaaS the Python dependencies are installed alongside the
  // server in dist/pylibs.  Propagate PYTHONPATH so the child process can
  // locate them (we no longer use -I / isolated mode — the PYTHON_BRIDGE
  // script already sanitises sys.path to remove the cwd).
  const pylibs = path.resolve(path.dirname(process.argv[1] || ''), 'pylibs')
  try {
    await fsAccess(pylibs)
    env.PYTHONPATH = [pylibs, agentRoot, env.PYTHONPATH || ''].filter(Boolean).join(path.delimiter)
  } catch {
    // dist/pylibs not present (dev environment) — rely on system site-packages
  }
  const pythonArgs = [
    ...command.argsPrefix,
    '-c',
