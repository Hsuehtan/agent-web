...
class AgentClient {
    ...
    async connect(port?: number): Promise<void> {
        const actualPort = port ?? parseInt(process.env.DEPLOY_RUN_PORT || process.env.PORT || '5000', 10)
            ...
            if (this.contextEngine && this.storage) {
                try {
                    ...
                    const ctx = await this.contextEngine.buildContext({
                        ...
                        compression,
                        profile: this.profile,
                        agentMentionEnabled: roomInfo.agentMentionEnabled,
                        mentionDepthLimit: roomInfo.mentionDepthLimit,
                        customRules: roomInfo.customRules,
...
export class AgentClients {
    ...
    async processMentions(roomId: string, msg: MentionMessage): Promise<void> {
        // Depth truncation: only check for agent-origin mentions
        // user-origin mentions are never depth-limited
        if (msg.origin === 'agent') {
            ...
            if (currentDepth >= depthLimit) {
                ...
                return   // hard truncation, no routing
            }
        }
    ...
    /**
     * Clear all agent-origin mention records from the queue for a given room.
     * Already-processing mentions are not affected (they will complete normally).
     */
    clearAgentMentionQueue(roomId: string): void {
        for (const key of Array.from(this._mentionQueue.keys())) {
            ...
            if (!queue) continue
            // Only remove origin === 'agent' records, keep user-origin
...
function nextMentionDepth(msg: MentionMessage): number {
    ...
    // Only increment depth for agent-origin mentions
    // user-origin mentions reset depth (do not count toward limit)
