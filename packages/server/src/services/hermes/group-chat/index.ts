...
class ChatStorage {
    ...
    init(): void {
        ...
        // Migration: add new columns to existing gc_rooms table
        const newColumns: Array<{ name: string; def: string }> = [
            { name: 'agentMentionEnabled', def: 'INTEGER NOT NULL DEFAULT 0' },
            { name: 'mentionDepthLimit', def: 'INTEGER NOT NULL DEFAULT 10' },
            { name: 'customRules', def: "TEXT NOT NULL DEFAULT ''" },
        ]
        for (const col of newColumns) {
            try {
                const existing = (db.prepare('PRAGMA table_info(gc_rooms)').all() as Array<{ name: string }>).find(c => c.name === col.name)
                if (!existing) {
                    db.exec(`ALTER TABLE gc_rooms ADD COLUMN ${col.name} ${col.def}`)
                    logger.info(`[ChatStorage] migrated gc_rooms: added column ${col.name}`)
                }
            } catch (err: any) {
                logger.warn(`[ChatStorage] migration for gc_rooms.${col.name} skipped: ${err.message}`)
            }
        }
        _tablesEnsured = true
    }
    ...
    getRoom(roomId: string): RoomInfo | undefined {
        return this.db()?.prepare('SELECT id, name, inviteCode, triggerTokens, maxHistoryTokens, tailMessageCount, totalTokens, sessionSeed, agentMentionEnabled, mentionDepthLimit, customRules FROM gc_rooms WHERE id = ?').get(roomId) as any
    }

    getRoomByInviteCode(code: string): RoomInfo | undefined {
        return this.db()?.prepare('SELECT id, name, inviteCode, triggerTokens, maxHistoryTokens, tailMessageCount, totalTokens, sessionSeed, agentMentionEnabled, mentionDepthLimit, customRules FROM gc_rooms WHERE inviteCode = ?').get(code) as any
    }

    getAllRooms(): RoomInfo[] {
        return (this.db()?.prepare('SELECT id, name, inviteCode, triggerTokens, maxHistoryTokens, tailMessageCount, totalTokens, sessionSeed, agentMentionEnabled, mentionDepthLimit, customRules FROM gc_rooms ORDER BY id').all() || []) as any[]
    }
    ...
    getRoomsForProfiles(profiles: string[]): RoomInfo[] {
        ...
        return (this.db()?.prepare(
            `SELECT DISTINCT r.id, r.name, r.inviteCode, r.triggerTokens, r.maxHistoryTokens, r.tailMessageCount, r.totalTokens, r.sessionSeed, r.agentMentionEnabled, r.mentionDepthLimit, r.customRules
             FROM gc_rooms r
    ...
    saveRoom(id: string, name: string, inviteCode?: string, config?: { triggerTokens?: number; maxHistoryTokens?: number; tailMessageCount?: number }, customRules?: string): void {
        this.db()?.prepare(
            'INSERT OR IGNORE INTO gc_rooms (id, name, inviteCode, triggerTokens, maxHistoryTokens, tailMessageCount, agentMentionEnabled, mentionDepthLimit, customRules) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)'
        ).run(id, name, inviteCode || null, config?.triggerTokens ?? 100000, config?.maxHistoryTokens ?? 32000, config?.tailMessageCount ?? 10, 0, 10, (customRules || '').slice(0, 2000))
    }

    updateRoomConfig(roomId: string, config: { triggerTokens?: number; maxHistoryTokens?: number; tailMessageCount?: number; agentMentionEnabled?: number; mentionDepthLimit?: number; customRules?: string }): void {
        const sets: string[] = []
        const vals: any[] = []
        ...
        if (config.agentMentionEnabled !== undefined) { sets.push('agentMentionEnabled = ?'); vals.push(config.agentMentionEnabled) }
        if (config.mentionDepthLimit !== undefined) { sets.push('mentionDepthLimit = ?'); vals.push(config.mentionDepthLimit) }
        if (config.customRules !== undefined) { sets.push('customRules = ?'); vals.push(config.customRules) }
...
export class GroupChatServer {
    ...
    private onConnection(socket: Socket): void {
        ...
        socket.on('toggle_agent_mention', (data: { roomId: string; enabled: boolean }, ack?: (response?: unknown) => void) => this.handleToggleAgentMention(socket, data, ack))
        socket.on('update_custom_rules', (data: { roomId: string; customRules: string }, ack?: (response?: unknown) => void) => this.handleUpdateCustomRules(socket, data, ack))
    ...
    private handleMessage(socket: Socket, data: Partial<ChatMessage> & { roomId?: string; content: string | Array<Record<string, unknown>>; id?: string; mentionDepth?: number }, ack?: (res: any) => void): void {
        ...
        const shouldRouteMentions =
            savedMsg.role === 'user'                        // user messages always trigger
            || (agentMentionEnabled && savedMsg.role === 'assistant')  // agent messages trigger when enabled

        // User sends new message: clear all agent-origin mentions in the queue
        ...
        if (shouldRouteMentions) {
            ...
            this.agentClients.processMentions(roomId, {
                ...
                mentionDepth,
                origin: savedMsg.role === 'assistant' ? 'agent' : 'user',
            }).catch((err) => {
    ...
    private handleToggleAgentMention(socket: Socket, data: { roomId: string; enabled: boolean }, ack?: (res: any) => void): void {
        ...
        if (!enabled) {
            // When disabling, clear all agent-origin mentions in queue
            this.agentClients.clearAgentMentionQueue(roomId)
        }

        // Notify all room members about the toggle
        this.nsp.to(roomId).emit('agent_mention_toggled', { roomId, enabled })
        ack?.({ ok: true })
    }

    private handleUpdateCustomRules(socket: Socket, data: { roomId: string; customRules: string }, ack?: (res: any) => void): void {
        ...
        // Length protection: truncate to 2000 characters
