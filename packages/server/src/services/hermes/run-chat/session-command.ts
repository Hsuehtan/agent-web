...
export interface SessionCommandResult {
  /** Whether the command was fully handled locally (no further processing needed). */
  handled: boolean
}
...
export async function handleSessionCommand(
  ...
  if (!COMMAND_ALIASES[command.rawName]) {
    // Not a built-in session command — pass through to the agent so it can
    // be resolved as a skill slash command (e.g. /remotion, /excalidraw).
