...
import { handleSessionCommand, isSessionCommand, parseSessionCommand, COMMAND_ALIASES } from './session-command'
...
export class ChatRunSocket {
  ...
  private onConnection(socket: Socket) {
    ...
    socket.on('run', async (data: {
      ...
      if (data.session_id) {
        ...
        if (command && source === 'cli') {
          try {
            const result = await handleSessionCommand(data.session_id, command, {
              nsp: this.nsp,
              socket,
            ...
            // If the command was handled locally (e.g. /usage, /abort), stop here.
            // If not handled (e.g. /remotion — a skill slash command), fall through
            // so the message is forwarded to the agent for skill resolution.
    ...
    socket.on('resume', async (data: { session_id?: string }) => {
      ...
      try {
        ...
        logger.warn(err, '[chat-run-socket] resume failed for session %s', sid)
        socket.emit('resumed', {
          session_id: sid,
          messages: [],
          isWorking: false,
  ...
  private async handleRun(
    ...
    const source = resolveRunSource(data.source, data.session_id)
    // Only skip built-in session commands (like /usage, /abort).
    // Skill slash commands (like /remotion) should pass through to the agent.
    if (data.session_id && source === 'cli' && isSessionCommand(data.input)) {
      const parsed = parseSessionCommand(data.input)
      if (parsed && COMMAND_ALIASES[parsed.rawName]) return
    }
