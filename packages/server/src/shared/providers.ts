...
export const _RAW_PROVIDER_PRESETS: ProviderPreset[] = [
  {
...
/**
 * OEM-filtered provider presets.
 * Providers listed in `oemConfig.providers.excludeValues` are removed at load time.
 * All consumers should use `PROVIDER_PRESETS` (the filtered version).
 * The unfiltered source is available as `_RAW_PROVIDER_PRESETS` if needed.
 */
export const PROVIDER_PRESETS = (() => {
  const exclude = oemConfig.providers.excludeValues
  if (!exclude.length) return _RAW_PROVIDER_PRESETS
  const set = new Set(exclude)
  return _RAW_PROVIDER_PRESETS.filter((p) => !set.has(p.value))
})()
